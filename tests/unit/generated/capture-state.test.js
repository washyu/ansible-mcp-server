...
describe('capture-state', () => {
  ...
  it('should execute successfully with valid parameters', async () => {
    const result = await global.testUtils.callMCPTool(server, 'capture-state', {
      components: ['vms', 'network'],
    ...
    if (result.success) {
      ...
      expect(result.output.toLowerCase()).toMatch(/captured|state|saved/);
    }
  });

  it.skip('should fail when missing required parameters', async () => {
    const result = await global.testUtils.callMCPTool(server, 'capture-state', {
      // Missing required params: components, outputDir
    });
  ...
  it('should validate parameter types', async () => {
    const result = await global.testUtils.callMCPTool(server, 'capture-state', {
      ...
      includeDiagrams: 'invalid', // Should be boolean
      captureConfigs: 'invalid', // Should be boolean
    });
  ...
  // TODO: Add more specific tests for capture-state
  // - Edge cases
